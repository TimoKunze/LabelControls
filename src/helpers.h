...
#ifdef UNICODE
	/// \brief <em>A custom hit-test flag to express that the specified point lies in the control's window rectangle, but not over a link</em>
	#define SLHT_TEXTORBACKGROUND 0x0001
	/// \brief <em>A custom hit-test flag to express that the specified point lies over a link</em>
	#define SLHT_LINK 0x0046
	/// \brief <em>A custom hit-test flag to express that the specified point lies above the control's window rectangle</em>
	#define SLHT_ABOVE 0x0100
	/// \brief <em>A custom hit-test flag to express that the specified point lies below the control's window rectangle</em>
	#define SLHT_BELOW 0x0200
...
HBITMAP LoadJPGResource(UINT jpgToLoad);
#ifdef UNICODE
	/// \brief <em>Retrieves whether a hyper link index is a valid one</em>
	///
	/// \param[in] linkIndex The link index to check.
	/// \param[in] pSysLink The \c SysLink object containing the link.
	///
	/// \return \c TRUE if the link index is valid; otherwise \c FALSE.
	...
	///
	/// \param[in] linkIndex The link index to check.
	/// \param[in] hWndSysLink The sys link control containing the link.
	///
	/// \return \c TRUE if the link index is valid; otherwise \c FALSE.
	BOOL IsValidLinkIndex(int linkIndex, HWND hWndSysLink);

	/// \brief <em>Describes a part of the window text of a \c SysLink control</em>
	///
	/// \sa ParseSysLinkText
	typedef struct DOCITEM
	{
		...
		///
		/// Parses the window text of a \c SysLink control and extracts the individual document parts like
		/// text parts and links (including URL and ID).
		///
		/// \param[in] text The text to parse.
		/// \param[in,out] documentItems A vector of the document parts in the order they occur in the text.
		...
		///
		/// \param[in] text The text to parse.
		/// \param[in,out] documentItems A list of the document parts in the order they occur in the text.
		///                Each part is described by a \c DOCITEM struct.
		///
		/// \sa DOCITEM, SysLink
...
///
/// \sa TargetOLEDataObject::SetData, CreateBitmapFromDIB, CreateDIBV5FromBitmap,
///     <a href="https://msdn.microsoft.com/en-us/library/ms532284.aspx">BITMAPINFO</a>
...
/// \return A global memory object containing the DIB data.
///
/// \sa TargetOLEDataObject::SetData, CreateBitmapFromDIB, CreateDIBFromBitmap,
///     <a href="https://msdn.microsoft.com/en-us/library/ms532284.aspx">BITMAPINFO</a>,

...
LRESULT CommonProperties::OnListViewGetInfoTipNotification(int /*controlID*/, LPNMHDR pNotificationDetails, BOOL& /*wasHandled*/)
{
	...
	switch(pDetails->iItem) {
		case 0:
			ATLVERIFY(SUCCEEDED(StringCchCopy(pBuffer, pDetails->cchTextMax + 1, TEXT("MouseDown, MouseUp, MouseEnter, MouseHover, MouseLeave, LinkMouseEnter, LinkMouseLeave, MouseMove"))));
			break;
		case 1:
		...
		case 2:
			...
			case 4:
				...
				break;
			case 5:
				ATLVERIFY(SUCCEEDED(StringCchCopy(pBuffer, pDetails->cchTextMax + 1, TEXT("CustomizeTextDrawing"))));
				break;
		#endif
	}
	...
	for(UINT object = 0; object < m_nObjects; ++object) {
		LPUNKNOWN pControl = NULL;
			...
			if(m_ppUnk[object]->QueryInterface(IID_ISysLink, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
				...
				reinterpret_cast<ISysLink*>(pControl)->get_DisabledEvents(&disabledEvents);
				...
				SetBit(controls.disabledEventsList.GetItemState(2, LVIS_STATEIMAGEMASK), l, deTextChangedEvents);
				SetBit(controls.disabledEventsList.GetItemState(3, LVIS_STATEIMAGEMASK), l, deKeyboardEvents);
				SetBit(controls.disabledEventsList.GetItemState(4, LVIS_STATEIMAGEMASK), l, deCustomDraw);
				SetBit(controls.disabledEventsList.GetItemState(5, LVIS_STATEIMAGEMASK), l, deCustomizeTextDrawing);
				reinterpret_cast<ISysLink*>(pControl)->put_DisabledEvents(static_cast<DisabledEventsConstants>(l));
				pControl->Release();
			} else
		#endif
		if(m_ppUnk[object]->QueryInterface(IID_IWindowedLabel, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
			DisabledEventsConstants disabledEvents = static_cast<DisabledEventsConstants>(0);
			reinterpret_cast<IWindowedLabel*>(pControl)->get_DisabledEvents(&disabledEvents);
			LONG l = static_cast<LONG>(disabledEvents);
			SetBit(controls.disabledEventsList.GetItemState(0, LVIS_STATEIMAGEMASK), l, deMouseEvents);
			SetBit(controls.disabledEventsList.GetItemState(1, LVIS_STATEIMAGEMASK), l, deClickEvents);
			SetBit(controls.disabledEventsList.GetItemState(2, LVIS_STATEIMAGEMASK), l, deTextChangedEvents);
			reinterpret_cast<IWindowedLabel*>(pControl)->put_DisabledEvents(static_cast<DisabledEventsConstants>(l));
			pControl->Release();

		} else if(m_ppUnk[object]->QueryInterface(IID_IWindowlessLabel, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
			DisabledEventsConstants disabledEvents = static_cast<DisabledEventsConstants>(0);
			reinterpret_cast<IWindowlessLabel*>(pControl)->get_DisabledEvents(&disabledEvents);
			LONG l = static_cast<LONG>(disabledEvents);
			SetBit(controls.disabledEventsList.GetItemState(0, LVIS_STATEIMAGEMASK), l, deMouseEvents);
			SetBit(controls.disabledEventsList.GetItemState(1, LVIS_STATEIMAGEMASK), l, deClickEvents);
			SetBit(controls.disabledEventsList.GetItemState(2, LVIS_STATEIMAGEMASK), l, deTextChangedEvents);
			reinterpret_cast<IWindowlessLabel*>(pControl)->put_DisabledEvents(static_cast<DisabledEventsConstants>(l));
			pControl->Release();
		}
	}
...
void CommonProperties::LoadSettings(void)
{
	...
	// get the settings
	properties.numberOfSysLinks = 0;
	properties.numberOfWindowedLabels = 0;
	properties.numberOfWindowlessLabels = 0;
	...
	if(pDisabledEvents) {
		...
		for(UINT object = 0; object < m_nObjects; ++object) {
			...
			#ifdef UNICODE
				if(m_ppUnk[object]->QueryInterface(IID_ISysLink, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
					++properties.numberOfSysLinks;
					reinterpret_cast<ISysLink*>(pControl)->get_DisabledEvents(&pDisabledEvents[object]);
					pControl->Release();
				} else
			#endif
			if(m_ppUnk[object]->QueryInterface(IID_IWindowedLabel, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
				++properties.numberOfWindowedLabels;
				reinterpret_cast<IWindowedLabel*>(pControl)->get_DisabledEvents(&pDisabledEvents[object]);
				pControl->Release();

			} else if(m_ppUnk[object]->QueryInterface(IID_IWindowlessLabel, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
				++properties.numberOfWindowlessLabels;
				reinterpret_cast<IWindowlessLabel*>(pControl)->get_DisabledEvents(&pDisabledEvents[object]);
				pControl->Release();
			}
		}
		...
		controls.disabledEventsList.AddItem(2, 0, TEXT("TextChanged event"));
		controls.disabledEventsList.SetItemState(2, CalculateStateImageMask(m_nObjects, pl, deTextChangedEvents), LVIS_STATEIMAGEMASK);
		#ifdef UNICODE
			if(properties.numberOfSysLinks > 0) {
				controls.disabledEventsList.AddItem(3, 0, TEXT("Keyboard events"));
				controls.disabledEventsList.SetItemState(3, CalculateStateImageMask(m_nObjects, pl, deKeyboardEvents), LVIS_STATEIMAGEMASK);
				controls.disabledEventsList.AddItem(4, 0, TEXT("CustomDraw event"));
				controls.disabledEventsList.SetItemState(4, CalculateStateImageMask(m_nObjects, pl, deCustomDraw), LVIS_STATEIMAGEMASK);
				controls.disabledEventsList.AddItem(5, 0, TEXT("CustomizeTextDrawing event"));
				controls.disabledEventsList.SetItemState(5, CalculateStateImageMask(m_nObjects, pl, deCustomizeTextDrawing), LVIS_STATEIMAGEMASK);
			}
		#endif

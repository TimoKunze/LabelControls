...
HBITMAP LoadJPGResource(UINT jpgToLoad)
{
...
#ifdef UNICODE
	BOOL IsValidLinkIndex(int linkIndex, SysLink* pSysLink)
	{
		if(linkIndex < 0) {
			return FALSE;
		}

		HWND hWndSysLink = NULL;
		if(pSysLink) {
			OLE_HANDLE h = NULL;
			pSysLink->get_hWnd(&h);
			hWndSysLink = static_cast<HWND>(LongToHandle(h));
		}

		return IsValidLinkIndex(linkIndex, hWndSysLink);
	}

	BOOL IsValidLinkIndex(int linkIndex, HWND hWndSysLink)
	{
		if(linkIndex < 0 || !IsWindow(hWndSysLink)) {
			return FALSE;
		}
		...
		return SendMessage(hWndSysLink, LM_GETITEM, 0, reinterpret_cast<LPARAM>(&link));
	}
	...
	#endif
	{
		...
		const WCHAR breakChar = L' ';

		BOOL currentTextIsLink = FALSE;
		int tagLength = 0;
		int textLength = 0;
		int linkLength = 0;
		UINT urlLength = 0;
		UINT idLength = 0;
		LPCWSTR pTextStart = NULL;
		LPCWSTR pLinkText = NULL;
		LPCWSTR pFirstTag = NULL;
		LPCWSTR pID = NULL;
		LPCWSTR pURL = NULL;
		for(LPCWSTR pCurrentChar = text; *pCurrentChar != 0;) {
			if(*pCurrentChar == L'<') {
				if(!StrCmpNIW(pCurrentChar, L"<a", 2) && !currentTextIsLink) {
					...
					if(*(pCurrentChar + 2) == L'>') {
						...
						isValidLink = TRUE;
						isValidParam = TRUE;
						pFirstTag = pCurrentChar;
						linkLength = 0;
						pID = NULL;
						pURL = NULL;
					}	else if(*(pCurrentChar + 2) == breakChar) {
						// we expect parameters, parse them
						LPCWSTR* ppCurrentParameter = NULL;
						LPCWSTR pTmp;
						UINT* pCurrentParameterLength = NULL;

						tagLength = 3;
						pTmp = pCurrentChar + tagLength;
						pID = NULL;
						pURL = NULL;

CheckParameter:
						// compare the current position with all known parameters
						if(!StrCmpNIW(pTmp, L"href=\"", 6)) {
							tagLength += 6;
							isValidParam = TRUE;
							ppCurrentParameter = &pURL;
							pCurrentParameterLength = &urlLength;
						} else if(!StrCmpNIW(pTmp, L"id=\"", 4)) {
							tagLength += 4;
							isValidParam = TRUE;
							ppCurrentParameter = &pID;
							pCurrentParameterLength = &idLength;
						} else {
							isValidParam = FALSE;
						}

						if(isValidParam) {
							// We got a known parameter, now search until the next " character. If we can't find a " character, there's a syntax error and we just assume it's text.
							isValidParam = FALSE;
							*ppCurrentParameter = pCurrentChar + tagLength;
							*pCurrentParameterLength = 0;

							for(pTmp = *ppCurrentParameter; *pTmp != 0; pTmp++) {
								tagLength++;
								if(*pTmp == L'\"') {
									isValidParam = TRUE;
									pTmp++;
					...
					if(isValidLink && isValidParam) {
						// The <a ...> tag appears to be valid. Save all information so we can add the link if we find a valid </a> tag later.
						currentTextIsLink = TRUE;
						pLinkText = pCurrentChar + tagLength;
						linkLength = 0;
						pFirstTag = pCurrentChar;
					} else {
						tagLength = 1;
						...
						pURL = NULL;
						if(!pTextStart) {
							pTextStart = pCurrentChar;
						}
					}
				} else if(!StrCmpNIW(pCurrentChar, L"</a>", 4) && currentTextIsLink && pFirstTag) {
					// there's a <a...> tag opened, first add the previous text, if present
					if(pTextStart && textLength > 0 && pFirstTag > pTextStart) {
						UINT l = min(pFirstTag - pTextStart, lstrlenW(pTextStart));
						DOCITEM docItem;
						docItem.isLink = FALSE;
						docItem.text = pTextStart;
						docItem.text = docItem.text.Left(l);
						#ifdef USE_STL
							documentItems.push_back(docItem);
						#else
							documentItems.AddTail(docItem);
						#endif
						pTextStart = NULL;
						textLength = 0;
					}
						
					// now it's time to add the link to the element
					pCurrentChar += 4;
					if(pLinkText && linkLength > 0) {
						UINT l = min(linkLength, lstrlenW(pLinkText));
						DOCITEM docItem;
						docItem.isLink = TRUE;
						docItem.text = pLinkText;
						docItem.text = docItem.text.Left(l);
						if(currentTextIsLink) {
							// Copy the tag parameters
							if(pID) {
								l = min(idLength, static_cast<UINT>(lstrlenW(pID)));
								l = min(l, MAX_LINKID_TEXT - 1);
								docItem.ID = pID;
								docItem.ID = docItem.ID.Left(l);
							}
							if(pURL) {
								l = min(urlLength, static_cast<UINT>(lstrlenW(pURL)));
								l = min(l, L_MAX_URL_LENGTH - 1);
								docItem.URL = pURL;
								docItem.URL = docItem.URL.Left(l);
							}
						}
						#ifdef USE_STL
							documentItems.push_back(docItem);
		...
		if(pTextStart && textLength > 0) {
			UINT l = min(textLength, lstrlenW(pTextStart));
			DOCITEM docItem;
			docItem.isLink = currentTextIsLink;
			docItem.text = pTextStart;
			docItem.text = docItem.text.Left(l);
			if(currentTextIsLink) {
				// Copy the tag parameters
